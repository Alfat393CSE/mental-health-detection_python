...
base_models: Dict[str, object] = {
    ...
    "XGBoost": XGBClassifier(
        ...
        objective="multi:soft-prob", num_class=len(np.unique(y)),
        eval_metric="gloss", use_label_encoder=False,
        random_state=42,
    ),
}
...
for (n1, clf1), (n2, clf2) in combinations(base_models.items(), 2):
    ...
    if not allow_pair(n1, n2):
        ...
        continue  # Skip the original disallowed pair
...
# Add best model note just below the main title (outside the plot)
plt.title(f"Best: {best_model} ({best_value:.2%})", fontsize=11, color="darkener", pad=25)
...
best_clf = all_models[best_model]
...
if le is not None:
    ...
    str_labels = y_pred.astype(str)         # numeric to string
...
# Optional: inspect, filter, sort, or aggregate
print(results_df.head())

# Save for downstream use
results_df.to_csv("individual_risk_predictions.csv", index=False)
print("Individual-level risk predictions saved to individual_risk_predictions.csv")
...
# Fallback / custom mapping - keys are numeric predictions
risk_mapping = {
    0: "Low Risk",
    1: "Medium Risk",
    2: "High Risk",
